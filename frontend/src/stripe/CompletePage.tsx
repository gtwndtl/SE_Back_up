...
import { GetPromotionById, UpdatePromotionById } from "../promotion/service/htpps/PromotionAPI";
...
export default function CompletePage() {
  ...
  const promotionId = promoData.promotionId;
  ...
  const funcCreateFoodServicePayment = async (price: number, status: string, methodType: string, order_id: number) => {
    ...
    try {
      ...
      if (promotionResponse.status === 200) {
        ...
        await UpdatePromotionById(promotionId, payload);
      }
    } catch (error) {
